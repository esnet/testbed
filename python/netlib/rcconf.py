...
class RcLocal:
    def __init__(self,rootpath=""):
        ...
        self.filename = os.path.join(self.rootpath + "/etc/rc.d/rc.local")
        # Try first /etc/rc.d/rc.local and then /etc/rc.local
        if not os.path.exists(self.filename):
            self.filename = self.rootpath + "/etc/rc.local"
            if not os.path.exists(self.filename):
                    self.exists = False
            else:
                self.exists = True
...
for eth in [["eth2","eth10"],["eth3","eth11"],["eth4","eth12"],["eth5","eth13"],["eth20","eth14"],["eth21","eth15"]]:
    print eth
    conf.changeInterface(eth,view=True)
